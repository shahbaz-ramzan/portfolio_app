...
const Contact: React.FC = () => {
  ...
  // let serviceKey = 'service_1uy0gul';
  // let templatekey = 'template_4ku4doc';
  // let publicKey = 'GxfFi9muFqOWI9eyg';
  ...
  const notifySentForm: React.FormEventHandler<HTMLFormElement> = async (e) => {
    ...
    try {
      ...
      fetch(`https://easy-tan-python-garb.cyclic.app/email/sendEmail`, {
        method: "POST",
...
const [sender, setSender] = ${name?.slice(0, 12)} ${name?.length >= 12 ? '...':''} ${lastUpdatedField === "name" ? (cursorBlink ? "|" : " ") : ""
    }🚀";
const [recipient, setRecipient] = "${email?.slice(0, 7)} ${email?.length >= 7 ? '...':''} ${lastUpdatedField === "email" ? (cursorBlink ? "|" : " ") : ""
    }📧";
const [subject, setSubject] = \n"${subject?.slice(0, 40)} ${subject?.length >= 40 ? '...':''} ${lastUpdatedField === "subject" ? (cursorBlink ? "|" : " ") : ""
    }✨";
const [message, setMessage] = 
\`Hello, intrepid traveler! 👋\n
Across the cosmos, a message for you:\n
"${wordWrap(message.slice(0, 40), 40, " ")} ${message.length >= 40 ? '...':''} ${lastUpdatedField === "message" ? (cursorBlink ? "|" : " ") : ""
    }"\n
